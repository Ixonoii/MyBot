...
client.on("message", function (message) {
    ...
    if (args[0].toLowerCase() === prefix + "whois") {
        ...
        .addField("**Roles:**", member.roles.map(r => `${r}`).join(' | '), false)
        .setTimestamp()
        message.channel.send(Success)
    }
})
...
client.on("message", message => {
    ...
    if(args[0].toLowerCase() === prefix + 'user') {
        var NoSuggestionEntered = new Discord.RichEmbed()
        .setColor("0xf35353")
        ...
        var Success = new Discord.RichEmbed()
        .setColor("0x38ee0e")
        .setTitle( emoji("689538521161138177") + "All the messages will now be sent to the user will the following ID: " + IDTyped)
        .setTimestamp()
        message.channel.send(Success)
        message.channel.setTopic(IDTyped)
    }
})
...
client.on("message", message => {
    ...
    if(args[0].toLowerCase() === prefix + 'send') {
        var NoSuggestionEntered = new Discord.RichEmbed()
        .setColor("0xf35353")
        .setTitle( emoji("689538472758870111") + "You must enter a message.")
        if (!message.member.hasPermission("ADMINISTRATOR")) return
        if(!message.guild.name === "MyBot Support Server") return
        if(!message.member.roles.some(r=>["Founder | Developer"].includes(r.name)) ) return
        if(!message.member.roles.some(r=>["."].includes(r.name)) ) return
        ...
        var Success = new Discord.RichEmbed()
        .setColor("0x38ee0e")
        .setTitle( emoji("689538521161138177") + message.author.username + ": " + IDTyped)
        .setTimestamp()
        var Success2 = new Discord.RichEmbed()
        .setColor("0x38ee0e")
        .setTitle( emoji("689538521161138177") + IDTyped)
        .setFooter("This message was sent by a member of the MyBot staff team and cannot be answered.")
        .setTimestamp()
        message.channel.send(Success)
        client.users.get(message.channel.topic).Send
    }
})
...
client.on("message", message => {
    ...
    if(args[0].toLowerCase() === prefix + 'remove') {
        ...
        .setColor("0xf35353")
        .setTitle( emoji("689538521161138177") + "Hello " + client.users.get(message.channel.topic).username + ". The staff team decided to remove MyBot from your Discord server. This may be due to non-compliance with Discord ToS, or the rules of use of MyBot. If you think this is an error, please contact a staff member by joining our Discord server.")
        .setTimestamp()
        message.channel.send(Success)
        client.guilds.get(IDTyped).owner.send(Notif).then(client.guilds.get(IDTyped).leave())
    }
})
